...
trait IsPublishableGroupEvent
{
    ...
    public function mapGroupForMessage($withMembers = true, $withGenes = true)
    {
        ...
        if ($withMembers) {
            $cocService = app(CocService::class);
            $data['members'] = $group->members()->isActive()->with(['person', 'person.credentials', 'person.institution', 'roles', 'latestCoi', 'person.latestCocAttestation'])->get()->map(function ($member) use ($cocService) { return $this->mapMemberForMessage($member, false, $cocService); })->toArray();
        }
