    ...
    public function mapGroupForMessage($withMembers = true, $withGenes = true)
    {
        ...
        if ($group->isEp) {
            ...
            if ($group->isVcep || $group->isScvcep) {
                ...
                $epData[$group->type->name . '_definition_approval']             = $ep->step_1_approval_date;
                $epData[$group->type->name . '_draft_specification_approval']    = $ep->step_2_approval_date;
                $epData[$group->type->name . '_pilot_approval']                  = $ep->step_3_approval_date;
                $epData[$group->type->name . '_final_approval']                  = $ep->step_4_approval_date;
            }
            if ($group->isGcep) {
