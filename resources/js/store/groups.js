...
export const actions = {
    ...
    async getGenes({ commit, getters, dispatch }, group) {
        ...
        if (geneSymbols.length && group.is_gcep) {
            ...
            item.expert_panel.genes = genes.map(g => {
                const plan = g.plan || {};
                const localCid = plan?.curation_id != null ? String(plan.curation_id) : null;
                const gt = localCid ? gtById[localCid] : undefined;

                // local snapshot key (from GPM) vs server hash (from GT)
                const localKey = plan?.checkKey || null;
                const remoteKey = gt?.checkKey || null;
    ...
    async discardScopeOfWorkRevision(_, { groupUuid, revisionUuid }) {
        const response = await api.post(`${baseUrl}/${groupUuid}/scope-of-work/revisions/${revisionUuid}/discard`);
        return response.data;
    },

    async discardScopeOfWorkChange(_, { groupUuid, revisionUuid, changeId }) {
        const response = await api.post(`${baseUrl}/${groupUuid}/scope-of-work/revisions/${revisionUuid}/changes/${changeId}/discard`);
        return response.data;
    },
