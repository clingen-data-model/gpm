...
async function save(e) {
	...
	try {
		...
		emit('saved')
		alertsStore.pushSuccess('ClinVar ID updated.')
		editing.value = false
	} catch (err) {
		if (isValidationError(err)) {
			errors.value = err.response?.data?.errors || {}
		} else {
			alertsStore.pushError(err?.response?.data?.message || 'Failed to update ClinVar Organization ID.')
		}
	} finally {
		saving.value = false
	}
...
<template>
  ...
  <form-container>
    <input-row
      v-model="clinvarOrgID"
      label="ClinVar Organization ID"
      :errors="errors.clinvar_org_id"
      :disabled="!editing"
      placeholder="ClinVar Organization ID"
      :max-length="15"
      label-width-class="w-48"
      input-class="w-32"
    >
      <template #after-input>
        <span v-if="!editing && !clinvarOrgID" class="text-gray-500 italic">Not set</span>
      </template>
    </input-row>
    <button-row v-if="editing">
      <button type="button" class="btn white" @click="cancelEdit">
        Cancel
      </button>
