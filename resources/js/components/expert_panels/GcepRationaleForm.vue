...
export default {
    ...
    computed: {
      ...
      htmlRationale() {
          return htmlFromMarkdown(this.group.expert_panel.prioritization_rationale || '');
      },
      expertPanel() {
      ...
      shouldShowRationale() {
        return this.group?.is_gcep && this.tier1CuratedGenesCount > 0;
      },
    },
    methods: {
      isCurated(g) {
...
<template>
  <div v-if="shouldShowRationale">
    ...
    <div class="mt-2 text-sm">
      <div role="note" class="mt-3 border-l-4 border-amber-400 bg-amber-50 px-3 py-2 text-sm text-amber-900 rounded-md dark:border-amber-400 dark:bg-amber-900/20 dark:text-amber-100">
      ...
      <transition name="fade" mode="out-in">
        ...
        <div v-else class="border-2 mt-8 p-2">
          <div v-if="group.expert_panel.prioritization_rationale" v-html="htmlRationale" />
