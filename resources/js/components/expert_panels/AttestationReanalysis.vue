			...
			<li>
				<input-row :errors="errors.reanalysis_review_lp" :hide-label="true">
					<checkbox v-model="group.expert_panel.reanalysis_review_lp" :disabled="disabled" @update:model-value="$emit('update'), checkCompleteness()">
						{{ group?.type?.display_name }}s are expected to re-review all {{ group.is_vcep ? 'LP' : 'Likely Oncogenic' }} and VUS classifications made by the EP at least every 2 years to see if new evidence has emerged to re-classify the variants
					</checkbox>
				</input-row>
			</li>
			<li>
