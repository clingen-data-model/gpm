<template>
  <div class="space-y-4">
    <div v-if="!history">Loading Scope of Work history...</div>
    <div v-else-if="history.versions.length === 0" class="well">No Scope of Work versions have been created yet.</div>

    <template v-else>
      <div v-for="(version, index) in history.versions" :key="version.uuid" class="rounded border bg-white p-4">
        <div class="flex items-start justify-between">
          <div>
            ...
            <!-- <button
              v-if="previousVersion(history, index)"
              type="button"
              class="btn btn-xs mt-2"
              @click="$emit('compare', {
                fromVersion: previousVersion(history, index),
                toVersion: version,
              })"
            >
            ...
            <div class="text-sm text-gray-600">
              <template v-if="version.base_version">Based on version {{ version.base_version.version_label }}.</template>
              <template v-if="version.submission">Submission: {{ version.submission.type }} — {{ version.submission.status }}.</template>
            </div>
...
function previousVersion(history, index) {
  return history?.versions?.[index + 1] || null;
}
</script>