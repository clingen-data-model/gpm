    ...
    <div>
        <div class="mb-4">
            ...
            <input-row
                ...
                :disabled="!canEdit"
                :label="`${group.type.display_name} Standardized Review Process`"
                vertical
                @update:modelValue="$emit('update')"
            />
        </div>
        <p class="text-sm mb-0">
            Note: For all variants approved by either of the processes described
            above, a summary of approved variants should be sent to ensure that
            any members absent from a call have an opportunity to review each
            variant. The summary should be emailed to the full
            {{ group.type.display_name }} after the call and should summarize
