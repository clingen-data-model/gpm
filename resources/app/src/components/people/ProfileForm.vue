...
const canEditAllFields = computed(() => {
    return hasPermission('people-manage') || userIsPerson(props.person)
})
...
<template>
    <div>
        ...
        <input-row label="First Name" v-model="profile.first_name" :errors="errors.first_name" />

        <input-row label="Last Name" v-model="profile.last_name" :errors="errors.last_name" />
        <input-row label="Email" v-model="profile.email" :errors="errors.email" />

        <input-row label="Institution" :errors="errors.institution_id">
            <InstitutionSearchSelect v-model="profile.institution_id" />
        </input-row>

        <input-row :errors="errors.credential_ids">
                    ...
                    <popover hover arrow>
                        ...
                        <template v-slot:content>
                            <div style="max-width: 500px">
                                ...
                                <p>
                                    Credentials are any degrees or professional certifications you have earned.
                                    Typically, these will be the acronyms you list after your name on your CV.
                                </p>
                                <p>
                                    We recommend choosing one or more options already in the list of credentials, but if
                                    you do not see yours feel free to add it.
                                </p>
                                <p>
                                    Please do not include current job titles or specific. If you would like to add that
                                    information to your profile please do so in the biography field below.
                                </p>
                            </div>
                        </template>
                    </popover>
        ...
        <input-row :errors="errors.expertise_ids">
            <template v-slot:label>
                Area of Expertise
                <div>
                    <popover hover arrow>
                        ...
                        <template v-slot:content>
                            <div style="max-width: 500px">
                                ...
                                <p>
                                    We use this information to determine if the make up of gene and variant expert
                                    panels includes the breadth of expertise needed to do it's work.
                                </p>
                                <p>
                                    Please choose the option from list that best describes your area of expertise. If
                                    you don't feel any of these options fit, select 'None'.
                                </p>
                            </div>
                        </template>
                    </popover>
        ...
        <input-row v-if="canEditAllFields" label="Biography" v-model="profile.biography" :errors="errors.biography"
            type="large-text" />
        ...
        <input-row v-if="canEditAllFields" label="Country" v-model="profile.country_id" type="select"
            :options="countries" :errors="errors.country_id" />

        <input-row v-if="canEditAllFields" label="Phone" v-model="profile.phone" :errors="errors.phone" />
