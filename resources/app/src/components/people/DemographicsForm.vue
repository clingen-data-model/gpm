<template>
    <div class="centered-container">
      <h1>ClinGen Profile Demographics</h1>
      ...
      <section>
        ...
        <p>The current list of countries comes from (<a
            href="https://www.iso.org/iso-3166-country-codes.html" target="_blank">the international standard ISO 3166 country codes </a>). We recognize that this
        ...
        <p> The options below come from updates by the  (<a href="https://www.census.gov/"
            ...
            href="https://www.researchallofus.org/data-tools/survey-explorer/the-basics-survey/" target="_blank">the All Of Us Basic Survey Questions</a>). We understand that these options do not capture the diversity of racial and ethnic identities in
        ...
        <p>We ask the following questions about identity to understand participation across the ClinGen ecosystem. The
          framing of the questions around sex and gender reflects recommendations made by (<a href="https://nap.nationalacademies.org/read/26424/chapter/1#xi"
            target="_blank">the National Academies of
  ...
  <li>Were or currently are homeless, as defined by the  (<a
            href="https://nche.ed.gov/mckinney-vento/" target="_blank">McKinney-Vento Homeless Assistance Act</a>);</li>
  <li>Were or currently are in the foster care system, as defined by the  (<a
            href="https://www.acf.hhs.gov/cb/focus-areas/foster-care" target="_blank">Administration for Children and Families</a>);</li>
  <li>Were eligible for the  for two or more years (<a
            href="https://www.fns.usda.gov/school-meals/income-eligibility-guidelines" target="_blank">Federal Free and Reduced Lunch Program</a>);</li>
   <li>Have/had no parents or legal guardians who completed a bachelor’s degree (<a
    ...
    <li>Were or currently are eligible for  (<a
            href="https://www2.ed.gov/programs/fpg/eligibility.html" target="_blank">Federal Pell grants</a>)</li>
    <li>Received support from the  as a
          parent or child (<a href="https://www.fns.usda.gov/wic/wic-eligibility-requirements"
            target="_blank">Special Supplemental Nutrition Program for Women, Infants and Children (WIC)</a>);</li>
    
</ol>
<br>

          Grew up in one of the following areas: <br>
          a) a U.S. rural area, as designated by (<a href="https://data.hrsa.gov/tools/rural-health" target="_blank">the Health Resources and Services Administration (HRSA) Rural Health
          Grants Eligibility Analyzer </a>);
          or<br>
          
          b) a Centers for Medicare and Medicaid Services-designated (<a
            href="https://www.qhpcertification.cms.gov/s/LowIncomeandHPSAZipCodeListingPY2020.xlsx?v=1" target="_blank">Low-Income and Health Professional Shortage Areas
        ...
        <div v-for="occupation in occupations" :key="occupation.value" style="display: flex;">
  ...
  <script setup name="DemographicsForm">
  var Vue = require('vue')
  import axios from 'axios'
  import { ref, emits, computed, onMounted } from 'vue'
  import { useStore } from 'vuex'
  import Person from '@/domain/person'
  //const person="store.getters.currentUser.person"
...
//console.log(person.last_name)
  ...
  // logic to compute uuid and person

  //return {
      // access a state in computed function
 // person: computed(() => store.getters.currentUser.person),

 //alternate way to retrieve person
// person: computed(() => store.getters.['people/currentItem'];

  // uuid: computed(() => store.dispatch('people/getPerson', {uuid: this.uuid}));
  //  }
 ...
 //computed: {
  ...
  // }
   ...
   //   type: Object,
   //   required: true,
     // default: "store.getters.currentUser.person"
...
//public function asController(ActionRequest $request, Person $person)
 //   {
 //       $demoData = $request->only(['first_name', 'last_name', 'email', 'credential_ids', 'expertise_ids', 'institution_id']);
