<template>
    <div>
        ...
        <edit-icon-button class="btn btn-sm float-right" @click="editPerson"
        ...
        <div class="flex space-x-4">
            ...
            <div>
                <dictionary-row class="pb-2" label-class="w-40" v-for="key in ['name', 'email']" :key="key"
                    :label="titleCase(key)">
                    {{ person[key] }}
                </dictionary-row>
                ...
                <section class="mt-4 border-t pt-4">
                    <h3>Profile</h3>
                    <div>
                        <dictionary-row class="pb-2" label-class="w-40" label="Institution">{{ person.institutionName
                            }}</dictionary-row>
                        ...
                        <dictionary-row class="pb-2" label-class="w-40" label="Biography">{{ person.biography
                            }}</dictionary-row>
                    </div>
                </section>
                ...
                <section class="mt-4 border-t pt-4">
                    ...
                    <dictionary-row class="pb-2" label-class="w-40" label="Address">
                        <div>
                            ...
                            <div>
                                <span v-if="person.city">{{ person.city }},</span> <span v-if="person.state">{{
                                    person.state }}</span> <span v-if="person.zip">{{ person.zip }}</span>
                            </div>
                        </div>
                    </dictionary-row>
...
<dictionary-row class="pb-2" label-class="w-40"
    label="Primary Occupation">{{person.primaryOccupationName}}</dictionary-row>
...
<dictionary-row class="pb-2" label-class="w-40" label="Gender">{{person.genderName}}</dictionary-row>
                ...
                <section class="mt-4 border-t pt-4" v-if="hasPermission('people-manage')">
                    ...
                    <dictionary-row class="pb-2" label-class="w-40" label="User ID">
                        {{ person.user_id || 'Account not activated.' }}</dictionary-row>
                    <dictionary-row class="pb-2" label-class="w-40" v-if="person.invite" label="Invite Code">
                        {{ person.invite.code }}
                        &nbsp;
                        <span v-if="person.invite.redeemed_at"> redeemed on
                            {{ formatDate(person.invite.redeemed_at) }}</span>
                    </dictionary-row>

                    <dictionary-row class="pb-2" label-class="w-40" v-for="key in ['created_at', 'updated_at']"
                        :key="key" :label="key">
